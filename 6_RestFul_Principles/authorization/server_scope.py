...
def get_scopes_for_role(role):
    if role == "admin":
        return ["book:read", "book:write", "book:delete", "author:manage", "user:manage", "borrow:manage"]
    else: 
        return ["book:read", "book:write", "borrow:self"]


def scope_required(required_scope):
    """Decorator kiểm tra xem token có chứa scope cụ thể không."""
    def wrapper(fn):
        @wraps(fn)
...
def login():
    ...
    user_role = user["role"]
    user_scopes = get_scopes_for_role(user_role)
...
def refresh():
    ...
    user_scopes = get_scopes_for_role(role)
...
def logout():
    """Xóa cookie Refresh Token để đăng xuất."""
...
def borrow_book(user_id):
    ...
    is_manager = "borrow:manage" in claims.get("scopes", [])

    if str(user_id) != current_user_id and not is_manager:
