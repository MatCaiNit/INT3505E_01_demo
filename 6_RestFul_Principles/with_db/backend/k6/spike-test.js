...
export let options = {
  stages: [
    { duration: "5s", target: 500 },  // spike lên
    { duration: "10s", target: 500 }, // giữ spike
    { duration: "5s", target: 50 },   // giảm nhanh
    { duration: "10s", target: 50 },  // ổn định
  ],
  thresholds: {
    http_req_duration: ["p(95)<5000"], // chấp nhận chậm hơn do spike
    http_req_failed: ["rate<0.2"],     // chấp nhận nhiều fail hơn
  },
};
...
export default function () {
    ...
    let testUsername = `user_${timestamp}_${random}`;
    let testEmail = `user_${timestamp}_${random}@example.com`;
    let jwtToken = "";
    let productId = "";
